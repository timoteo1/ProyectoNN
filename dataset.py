...
def filter_vocabulary(words, frequency, total, min_value=0.0, max_value=1.0):
    assert min_value < max_value, "Values error"
    word_list = list(words)
    word_list.sort(cmp=lambda x, y: frequency[x]-frequency[y])
    #total = reduce(lambda x, k: x + frequency[k], frequency, 0)
    min_index = reduce(lambda x, w: x + (1 if frequency[w]/total < min_value else 0), word_list, 0)
    max_index = reduce(lambda x, w: x + (1 if frequency[w]/total < max_value else 0), word_list, 1)
...
def mapping(words, start_at=1):
    ...
    to_index = defaultdict(lambda: 0, {words[i]: i+start_at for i in xrange(0, len(words))})
